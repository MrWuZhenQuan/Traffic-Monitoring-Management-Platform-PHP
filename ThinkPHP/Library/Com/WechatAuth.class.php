...
class WechatAuth {
    ...
    const MSG_TYPE_ARTICLES = 'mpnews';
    /* 二维码类型常量 */
    ...
    public function newsUpload($data){
        ...
        return $this->api('media/uploadnews', $data);
    }
    /**
     * 上传媒体资源
     ...
     * @param  array  $media   媒体素材id
     * @param  string $type    推送消息类型
     */
    public function massSendGroup($type,$media_id,$groupid = null){
        //基础数据
        
        if(!is_null($groupid)){
            $param['filter'] = array('is_to_all' => false,
                                     'group_id' => $groupid);
        }else{
            $param['filter'] = array('is_to_all' => true);
        }
        $param[$type] = array('media_id' => $media_id);
        $param['msgtype'] = $type;

        return $this->api2('message/mass/sendall', $param);
    }
    /**
     * 给指定用户推送信息预览
     * @modify qax
     ...
     * @param  string $type    推送消息类型
     */
    public function massPreview($type,$media_id,$openid){
        //基础数据
        $param = array('touser' => $openid,
                        $type => array('media_id' => $media_id),
                        'msgtype' => $type);

        return $this->api('message/mass/preview', $param);
    }
    /**
     * 给指定用户推送信息
     ...
     * @return array          api返回结果
     */
    protected function api2($name, $data = '', $method = 'POST', $param = ''){
        $params = array('access_token' => $this->accessToken);
